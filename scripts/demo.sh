...
psql "$PGURL" -c "select at::time(0), executed as tool, risk_tier as tier, rows_out, bytes_out, status
...
psql "$PGURL" -c "select approval_id, cp_id, action, params, status from fintech.approval order by approval_id desc limit 5"
